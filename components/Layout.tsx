    ...
    <header className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-3' : 'bg-white/90 backdrop-blur-sm md:bg-transparent py-5'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
          ...
          <Link to="/booking" className="bg-brand-600 text-white px-5 py-2 rounded-full font-medium hover:bg-brand-700 transition-all shadow-lg shadow-brand-200">
            Book Now
          </Link>
        </nav>
      ...
      {isOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-white shadow-lg py-4 px-4 flex flex-col gap-4 border-t max-h-[80vh] overflow-y-auto">
          ...
          <Link to="/ai-quote" className="text-slate-700 font-medium py-2 border-b border-slate-100">AI Quote</Link>
          <Link to="/smart-lockers" className="text-slate-700 font-medium py-2 border-b border-slate-100">Smart Lockers</Link>
          <Link to="/contact" className="text-slate-700 font-medium py-2 border-b border-slate-100">Contact</Link>
...
const LiveChatWidget = () => {
  ...
  return (
    <div className="fixed bottom-24 right-6 z-40 flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-80 bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="h-64 p-4 bg-slate-50 overflow-y-auto flex flex-col gap-3">
            <div className="self-start bg-white p-3 rounded-lg rounded-tl-none shadow-sm text-sm text-slate-700 max-w-[85%]">
