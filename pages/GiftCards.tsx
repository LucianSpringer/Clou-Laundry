...
import { GIFT_CARD_DESIGNS, COMPANY_NAME } from '../constants';
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8">
               <div className="relative rounded-xl overflow-hidden aspect-video shadow-lg mb-6 group">
                 ...
                 <div className="absolute bottom-6 left-6 text-white">
                   <p className="text-sm font-medium opacity-90">{COMPANY_NAME} Gift Card</p>
                   <p className="text-3xl font-bold">Rp {parseInt(amount).toLocaleString()}</p>
                 </div>
               </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {['100000', '250000', '500000'].map(val => (
                    <button 
                      ...
                      className={`py-3 px-2 rounded-lg font-bold text-sm border transition-all ${amount === val ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-slate-700 border-slate-200 hover:border-brand-400'}`}
                    >
